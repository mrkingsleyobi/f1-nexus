...
#[napi]
pub fn get_circuits() -> Vec<String> {
    vec![
        "monaco".to_string(),
        "spa".to_string(),
...
#[napi]
pub fn get_tire_compounds() -> Vec<String> {
    vec![
        "C0".to_string(),
        "C1".to_string(),
        "C2".to_string(),
...
///
/// # Arguments
/// * `params_json` - JSON string with optimization parameters
///
/// # Example
/// ```javascript
/// const result = optimizeStrategy(JSON.stringify({
///   track: "monaco",
///   total_laps: 78,
///   starting_fuel: 110.0,
///   position: 5,
...
/// ```
#[napi]
pub fn optimize_strategy(params_json: String) -> Result<String> {
    let input: OptimizeInput = serde_json::from_str(&params_json)
    ...
    if available_compounds.is_empty() {
        return Err(Error::from_reason("No valid tire compounds provided"));
    }
    ...
    let strategy = optimize_pit_strategy(&config)
        .map_err(|e| Error::from_reason(format!("Optimization failed: {}", e)))?;
...
/// ```
#[napi]
pub fn simulate_race(params_json: String) -> Result<String> {
    let input: SimulateInput = serde_json::from_str(&params_json)
        .map_err(|e| Error::from_reason(format!("Invalid input JSON: {}", e)))?;
    ...
    let strategy = RaceStrategy {
        id: format!("node-sim-{}", chrono::Utc::now().timestamp()),
        ...
        metadata: StrategyMetadata {
            ...
            contributing_agents: vec!["node".to_string()],
            version_hash: None,
...
/// ```
#[napi]
pub fn predict_tire_life(params_json: String) -> Result<String> {
    let input: TireLifeInput = serde_json::from_str(&params_json)
        .map_err(|e| Error::from_reason(format!("Invalid input JSON: {}", e)))?;

    let compound = parse_tire_compound(&input.compound)
        .ok_or_else(|| Error::from_reason("Invalid tire compound"))?;
